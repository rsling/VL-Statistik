...
\subsubsection{Transfer: Stichprobengröße}
...
\section{Transfer: Anteilswerte und Mittelwerte}
...
Um zu zeigen, dass dies tatsächlich die Varianz für Bernoulli-Stichproben ist, können wir die zunächst genauso berechnen wie für Mittelwerte, wenn wir den Anteilswert von Einsen $q$ als Mittelwert ansetzen.
...
\Sol{Wir kürzen $\sum\limits_{i=1}^{n}$ als $\sum$ ab.
\begin{align}
  &\frac{\sum(x_i-\bar{x})}{n}\\[0.5\baselineskip]
  \frac{\sum x_i}{n}&\cdot\frac{\sum(1-x_i)}{n}\\[0.5\baselineskip]
  q&\cdot(1-q)
\end{align}%
Und das war es eigentlich schon.
Die Definition der Anteilswerte für Einsen und Nullen des entspricht den Multiplikanden, wenn man die Summenformel auflöst.
Die Beispielrechnung:
  \begin{align}
    x_7&=[0,1,1,0,0,1,1,1,1,0,0]\\
    n_7&=10\\
    \bar{x_7}&=q_7=0.6\\
    s^2(x_7)&=\frac{\sum(x_{7_i}-\bar{x_7})^2}{n_7}=0.24=q_7(1-q_7)=0.6\cdot0.4\\
  \end{align}%
}
